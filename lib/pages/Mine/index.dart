...
class _MineViewState extends State<MineView> {
      ...
      child: Row(
        children: [
          Obx((){
            return CircleAvatar(
            radius: 26,
            backgroundImage: userController.userInfo.value.avatar.isNotEmpty
          ...
          Expanded(
            child: Column(
              ...
              children: [
                  ...
                  onTap: () {
                    if(userController.userInfo.value.id.isEmpty){
                      Navigator.pushNamed(context, '/login');
                    }
                    
                  },
                  child: Text(
                    userController.userInfo.value.id.isNotEmpty
                        ? userController.userInfo.value.account
                        : '请登录',
                    style: TextStyle(
                      fontSize: 18,
  ...
  List<GoodDetailItem> guessList = [];

  Map<String,dynamic> guessListParams = {
    "page": 1,
    "pageSize": 10,
  };
...
bool _harMore=true;//还有下一页
  //获取猜你喜欢列表
  void getGuessList() async {
    if(_isloading||!_harMore) return;//如果有人正在加载，直接返回
    _isloading=true;//设置为正在加载
    final res = await getGuessListAPI(guessListParams);
    guessList.addAll(res.items);
        setState(() {});
    if(guessListParams["page"]>=res.pages){
      _harMore=false;//没有下一页了
      return;//直接返回
    }
    guessListParams["page"]++;//页码加一
  ...
  Widget build(BuildContext context) {
    return CustomScrollView(
      ...
      slivers: [
        ...
        HmMoreList(recommendList: guessList),
      ],
    );
  }
}
